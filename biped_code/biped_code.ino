 ...
 * These parameters were optimized for speed while still maintaining a stable walking.
 * At first, we changed / looked at the code for only one parameter at each time to see what each parameter controls.
 * Later, we dynamically changed several parameters at a time such that the speed increased in each step.
 * Whenever we observed stability issues, we either went back to an earlier parameter configuration or changed other walking parameters such that the robot walked smooth again.
 * Finally, we measured the walking speed of the initial walking parameter configuration as well as the speed-optimized parameter configuration.
 * Our final configuration is the one set in the code.
*/

// s, h, t: 100, 65, 90 if hip is increased, torso must be decreased 33s for initial
// 22s: 100, 85, 140, 5, 3
// 22s: 110, 80, 130, 4, 2.7
int mySpeed = 110; // 100 also worked well; | from 0 to 255. 255 is full motor speed
int hipstep = 80; // We increased this parameter such that the robots makes larger steps.
int torsoStep = 130; /* The torso step had to be adapted such that walking stability was maintained. */
int myd = 4; //ms;  delay between two commands to the motors, it should be as small as possible.
int myd2 = 1000;//ms  // 1000
int myd3 = 20; /*20 a delay between to sensors readings sycles */
int hight_factor = 2.7; // 2 a multiplication factor for the hight of the feet in swing phase; We increased this parameter such that the height of the feet in the swing phase is enough for the hipstep parameter.
