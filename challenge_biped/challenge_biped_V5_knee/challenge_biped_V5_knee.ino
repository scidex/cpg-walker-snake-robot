...
void loop()
{
  ...
  float left_ankle = - 0.88*(3.0*rs_neuron[11].y + 2.0*rs_neuron[12].y + 1.0*rs_neuron[9].y);
  float right_ankle = 0.88*(3.0*rs_neuron[9].y + 2.0*rs_neuron[10].y + 1.0*rs_neuron[11].y);
  ...
  */
  ...
  LA_current = initial_pos[LA] - 30.0 - (hipstep * 0.7* left_ankle);
  ax12a.moveSpeed(LA, LA_current,0.3*mySpeed);
  delay(2);
  RA_current = initial_pos[RA] + 30.0 - (hipstep * 0.7* right_ankle);
  ax12a.moveSpeed(RA, RA_current,0.3*mySpeed);
  delay(2);
  print_motors();
